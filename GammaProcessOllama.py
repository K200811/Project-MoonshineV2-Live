...
OLLAMA_MODEL    = "deepseek-r1:latest"              # change to whichever model you have pulled
                                             # e.g. "qwen2.5:7b", "llama3.3:70b", "mistral", etc.
...
def ollama_chat(messages: list[dict], max_tokens: int = 8000, temperature: float = 0.2) -> str:
    ...
    response = requests.post(
        ...
        json=payload,
        timeout=30000,   # local models can be slow; increase if needed
    )
...
def InitalSuperGammaFunction(formula: str) -> str:
    messages = [
        {
            "role": "system",
            "content": (
                ...
                "ITS VERY IMPORTANT THAT THE NUMIONS FOR EACH ELEMENT MATCH WHAT IS EXPECTED FOR THE SPACE GROUP YOU SELECT"
    ...
    return ollama_chat(messages, max_tokens=8000, temperature=0.2)


# ──────────────────────────────────────────────────────────────────────────────
# Secondary Super Gamma Function  (iterative structure refinement)
# ──────────────────────────────────────────────────────────────────────────────

def secondarySuperGammaFunction(Energy, Forces, Stress, Dim, Group, Species, NumIons, formula) -> str:
    messages = [
        {
            "role": "system",
            "content": (
        ...
        },
        {
            "role": "user",
            "content": (
                "Prompt: Determine the PyXtal Parameters That should be used for the next crystal generation based on the values of these"
                " tests from the last Crystal. These were the values of the last crystal: "
                f"Dim: {Dim}, Space Group: {Group}, Species: {Species}, NumIons: {NumIons} "
                "Below are the tests that were run on the last structure and its results: "
                f"Energy: {Energy}, Forces: {Forces}, Stress: {Stress}. This is this crystal's formula: {formula}"
                "Figure out what the values for Dim, Space Group, Species, and NumIons should be for the next structure that's generated to create the most "
...
# ──────────────────────────────────────────────────────────────────────────────
# Formula / response parser
# ──────────────────────────────────────────────────────────────────────────────

def SubGammaFunction(text_to_search: str, start_keyword: str, end_keyword: str) -> str:
    if end_keyword == "end":
        extracted_text = ""
        if start_keyword in text_to_search:
            raw_answer = text_to_search.split(start_keyword)[-1]
            extracted_text = raw_answer.strip('#!"')
    ...
    if start_keyword in text_to_search and end_keyword in text_to_search:
        ...
        data["payload"]["logs"]["warnings"].append("KEYWORDS NOT IN RESPONSE - GAMMA PROCESS")
        sys.exit(1)
...
def GammaThreeFunction(file):
    ...
    analyser = SpacegroupAnalyzer(final_structure, symprec=0.01)
    print(f"Detected Space Group: {analyser.get_space_group_symbol()}")
...
if __name__ == "__main__":
    ...
    NUMER_OF_ITTERATIONS = 500

    Beta_Array = []
    GammaArray = []

    with open("BetaFile.json", "r", encoding="utf-8") as file:
    ...
    for entry in Beta_Array:
        ...
        # ── Uncomment to let the LLM choose parameters ──────────────────────
        InitalSuperGammaResult = InitalSuperGammaFunction(entry["Formula"])
        print(f"Formula: {entry['Formula']}")
        data["payload"]["logs"]["warnings"].append(f"InitalSuperGammaFunction Result {InitalSuperGammaResult}")
        ...
        SpeciesRaw = SubGammaFunction(InitalSuperGammaResult, "Species:", "NumIons").replace("'", '"')
        Species = json.loads(SpeciesRaw)
        print(f"Species:{Species}")

        NumIonsRaw = SubGammaFunction(InitalSuperGammaResult, "NumIons:", "end").replace("'", '"')
        NumIons = json.loads(NumIonsRaw)
        print(f"NumIons:{NumIons}")
        # ─────────────────────────────────────────────────────────────────────

        # Temp hardcoded values for testing without API calls
        # Dim     = 3
        # Group   = 122
        # Species = ["N", "H", "P", "O"]
        # NumIons = [4, 24, 4, 16]
        ...
        candidets = []
        chgnet = CHGNet.load()
        for ITTERATIONS in range(NUMER_OF_ITTERATIONS):
            print("\n--------------------Crystal Structure Generation---------------------------")
            try:
                crystal = GammaOneFunction(Dim, Group, Species, NumIons)
            except Exception as e:
                print(f"Error in PyXtal Generation {e}")
                continue

            print(f"made itteration number {ITTERATIONS}")

            PymatgenCrystal = crystal.to_pymatgen()
            prediction = chgnet.predict_structure(PymatgenCrystal)
            staticEnergy = prediction["e"]

            candidets.append((staticEnergy, crystal))

        candidets.sort(key = lambda x: x[0])
        best_candidates = candidets[:5]

        for i in range(len(best_candidates)):
            print("---------------- 5 Best Candident Information ------------")
            print(f"Candident Number {i}")
            crystalStructure = best_candidates[i][1]
            print(f"GammaOneFunction Crystal Structure Information: {crystalStructure}")
            print(f"Formula: {crystalStructure.formula}")
            print(f"Atoms: {crystalStructure.numIons}")
            print(f"Space Group: {crystalStructure.group}")

            crystalStructure.to_file(f"PyXtalStructure.cif_{entry["Formula"]}_{i}")
        ...
        for j in range (len(best_candidates)):
            ...
            GammaTwoResult = GammaTwoFunction(f"PyXtalStructure.cif_{entry["Formula"]}_{j}")
            ...
            if StressTestPass and ForcesTestPass and (Total_Energy / len(Relaxed_Structure)) < 0:
                ...
                export.write_file(f"relaxedStructure_{entry['Formula']}_{j}.cif")
                print(f"Ev/atom: {Total_Energy / len(Relaxed_Structure)}\n")
                print(f"{entry['Formula']} passed the coarse relaxation — structure written.\n")

                RunGammaThree = True

            # ── Fine CHGNet relaxation (Gamma Three) ─────────────────────────────
            RunGammaThree = True  # Temporary override — remove when coarse gate is trusted

            if RunGammaThree:
                print("\n------------------------- Running Gamma Three ---------------------")
                GammaThreeResult = GammaThreeFunction(f"relaxedStructure_{entry['Formula']}_{j}.cif")
                ...
                if StressTestPass and ForcesTestPass:
                    ...
                    export.write_file(f"Final_relaxedStructure_{entry['Formula']}_{j}.cif")
    ...
    # ---------------- END OF LOOP DATA CREATION -----------------

    cleanedGammaArray = list({entry["Formula"]: entry for entry in Beta_Array}.values())
    ...
    for i, entry in enumerate(cleanedGammaArray):
    ...
    with open("GammaFile", "w") as json_file:
        json.dump(cleanedGammaArray, json_file, indent=4)